...
"""everything except double echo is Brians code"""
...
def echo(sound1,sound2,delay,sample_length):
    ...
    for i in range(0,sample_length):
        ...
        if i>delay:
            echo=sound1[i-delay]*0.6
            values.append(echo+sound1[i])
    return values

def double_echo(sound1,delay, delay2,sample_length):
    values=[]
    for i in range(0,sample_length):
        values.append(sound1[i])
        if i>delay2:
            echo=sound1[i-delay2]*0.7
            values.append(echo+sound1[i])
        elif i>delay:
            echo=sound1[i-delay]*0.3
            values.append(echo+sound1[i])
...
echo = echo(tone_values_one,tone_values_one,60000,132000)
save_wave_file('Echo.wav',echo,SAMPLE_RATE)

echo2 = double_echo(tone_values_one,45000,90000,132000)
save_wave_file('Echo_double.wav',echo2,SAMPLE_RATE)